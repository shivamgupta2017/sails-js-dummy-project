 ...
 *     // -or-
 *     return res.unauthenticated(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function unauthenticated(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.unauthenticated()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.unauthenticated()` called with an Error:', optionalData);
    ...
    if (!_.isFunction(optionalData.toJSON)) {
      if (process.env.NODE_ENV === 'production') {
        ...
        return res.json(statusCodeToSet, { Error: optionalData.stack });
